...
function ProfileCard({
  ...
  return (
    // overflow-hidden 제거 → 이미지가 위로 자연스럽게 나옴
    <Card className="border-none shadow-lg">
      <div className="relative">
        {/* 상단 배너 바 (조금 더 높임) */}
        <div className="h-36 w-full bg-gradient-to-r from-law-navy to-law-gold/80" />
        {/* 프로필 이미지 (조금 더 아래로 내림, 윗부분 안 짤리게) */}
        <img
          src={photo}
          alt={alt}
          className="w-32 h-32 md:w-40 md:h-40 rounded-full object-cover shadow-xl ring-4 ring-white
                     absolute left-1/2 -translate-x-1/2 -bottom-16 md:-bottom-20 z-10"
        />
      </div>

      {/* 카드 본문 여백 확장 */}
      <CardContent className="pt-44 md:pt-52 p-8">
        {/* 이름 / 직함 */}
        ...
        <div className="grid md:grid-cols-3 gap-8">
          ...
          {specialties.length > 0 && (
            <div className="text-center md:text-left">
              ...
              <div className="space-y-2 text-gray-700">
                {specialties.map((t, i) => <p key={i}>{t}</p>)}
              </div>
            </div>
          )}
        </div>
